...
const initPositiveCubes = (myInput) => {
    ...
    for (let y = 0; y < myInput.length; y++) {
        ...
        for (let x = 0; x < line.length; x++) {
            if (line[x] === '#') positiveCubes.add(`${x}:${y}:0:0`);
        }
    }
    return positiveCubes;
}

const listNeighbors = (xyzw) => {
    const neighbors = [];
    const [rootx, rooty, rootz, rootw] = xyzw.split(':').map(e => +e);
    const range = [-1, 0, 1];
    ...
    range.map(z => {
        range.map(y => {
            range.map(x => {
                range.map(w => {
                    neighbors.push(`${x + rootx}:${y + rooty}:${z + rootz}:${w + rootw}`)
                })
            })
        })
    })
